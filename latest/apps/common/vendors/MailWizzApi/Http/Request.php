<?php
/**
 * This file contains the Cyber FisionApi_Http_Request class used in the Cyber FisionApi PHP-SDK.
 *
...
/**
 * Cyber FisionApi_Http_Request is the request class used to send the requests to the API endpoints.
 *
 * @author Serban George Cristian <cristian.serban@Cyber Fision.com>
 ...
 * @since 1.0
 */
class Cyber FisionApi_Http_Request extends Cyber FisionApi_Base
{
    /**
     * @var Cyber FisionApi_Http_Client the http client injected.
     */
    public $client;

    /**
     * @var Cyber FisionApi_Params the request params.
     */
    public $params;
     ...
     * Constructor.
     *
     * @param Cyber FisionApi_Http_Client $client
     */
    public function __construct(Cyber FisionApi_Http_Client $client)
    {
        $this->client = $client;
    }
    ...
    public function send()
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT , 'Cyber FisionApi Client version '. Cyber FisionApi_Http_Client::CLIENT_VERSION);
        ...
        if ($client->isPostMethod || $client->isPutMethod || $client->isDeleteMethod) {
            ...
            $params = new Cyber FisionApi_Params($client->paramsPost);
        ...
        $params = $this->params = new Cyber FisionApi_Params($curlInfo);
        ...
        if ($curlCode === 0 && $client->getResponseHeaders) {
            ...
            $params->add('headers', new Cyber FisionApi_Params($headers));
        }

        $decodedBody = array();
        if ($curlCode === 0 && !empty($body)) {
            $decodedBody = Cyber FisionApi_Json::decode($body, true);
        ...
        $params->add('body', new Cyber FisionApi_Params($decodedBody));

        $response = new Cyber FisionApi_Http_Response($this);
        $body = $response->body;
    ...
    protected function sign($requestUrl)
    {
        ...
        $params = new Cyber FisionApi_Params($specialHeaderParams);
