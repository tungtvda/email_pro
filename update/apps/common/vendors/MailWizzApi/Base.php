<?php
/**
 * This file contains the base class for the Cyber FisionApi PHP-SDK.
 *
...
/**
 * Cyber FisionApi_Base is the base class for all the other classes used in the sdk.
 * 
 * @author Serban George Cristian <cristian.serban@Cyber Fision.com>
 * @package Cyber FisionApi
 * @since 1.0
 */
class Cyber FisionApi_Base
{
    /**
    ...
    /**
     * @var Cyber FisionApi_Config the configuration object injected into the application at runtime
     */
    private static $_config;
    
    /**
     * @var Cyber FisionApi_Params the package registry that will hold various components
     */
    private static $_registry = array();
    
    /**
     * @var Cyber FisionApi_Params the registered event handlers
     */
     ...
     * 
     * @param Cyber FisionApi_Config $config
     */
    public static function setConfig(Cyber FisionApi_Config $config)
    {
        self::$_config = $config;
    }
     ...
     * 
     * @return Cyber FisionApi_Params
     */
    public function getRegistry()
    {
        if (!(self::$_registry instanceof Cyber FisionApi_Params)) {
            self::$_registry = new Cyber FisionApi_Params(self::$_registry);
        }
     ...
     * $components = array(
     *     'cache'=>array(
     *         'class'             => 'Cyber FisionApi_Cache_Sqlite',
    ...
    public function setComponent($componentName, array $config)
    {
        ...
        if ($component instanceof Cyber FisionApi_Base) {
    ...
    {
        if (!(self::$_eventHandlers instanceof Cyber FisionApi_Params)) {
            self::$_eventHandlers = new Cyber FisionApi_Params(self::$_eventHandlers);
        }
        
        if (!self::$_eventHandlers->contains($eventName) || !(self::$_eventHandlers->itemAt($eventName) instanceof Cyber FisionApi_Params)) {
            self::$_eventHandlers->add($eventName, new Cyber FisionApi_Params());
        }
